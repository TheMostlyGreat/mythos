...
**If you see `[skill-invocation-log] FAILED` above, or no `audit ✓` line at all**: STOP. Do not run /audit manually — that line is the only proof the done-gate accepts. Report the failure to the user (most likely cause: Codex's bash permission denied the injection) and ask them to resolve it before re-invoking /audit.
...
- `AGENTS.md`, `AGENTS.md` (root and subdirectories)
- `.Codex/AGENTS.md` (root and subdirectories)
...
| ---------- | ------------------------------------------------------------------- | -------- |
| Size limit | AGENTS.md/AGENTS.md: ~150-200 instructions; Cursor rules: 500 lines | warn     |
...
- [Anthropic Engineering](https://www.anthropic.com/engineering/Codex-best-practices)
...
Review existing test files for quality issues. Sample test files from the project and check against the iron laws and anti-patterns in `.Codex/skills/testing/SKILL.md`.
...
- [E001] Dead ref: `AGENTS.md` references missing file `src/foo.ts`
- [E002] Drift: `ARCHITECTURE.md` documents Redux, code uses Zustand

### Warnings (should review)

- [W001] Size: `AGENTS.md` has 245 instructions (recommended: 150-200)
