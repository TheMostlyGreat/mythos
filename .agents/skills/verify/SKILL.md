...
**If you see `[skill-invocation-log] FAILED` above, or no `verify ✓` line at all**: STOP. Do not run /verify manually — that line is the only proof the done-gate accepts. Report the failure to the user (most likely cause: Codex's bash permission denied the injection) and ask them to resolve it before re-invoking /verify.
