...
} from "../../_browser-chunks/chunk-4AB3IUR2.js";
import {
  _defineProperty,
...
var { window: globalWindow } = globalThis, Wrapper5 = styled9.div(({ theme: theme3 }) => ({
  ...
  let theme3 = useTheme(), data = useMemo(() => value && cloneDeep(value), [value]), hasData = data != null, [showRaw, setShowRaw] = useState4(!hasData), [parseError, setParseError] = useState4(null), readonly = !!argType?.table?.readonly, updateRaw = useCallback3(
    (raw) => {
      try {
...
// ../addons/docs/src/blocks/controls/index.tsx
var LazyColorControl = lazy(() => import("../../_browser-chunks/Color-QOVTYR6E.js")), ColorControl = (props) => React16.createElement(Suspense, { fallback: React16.createElement("div", null) }, React16.createElement(LazyColorControl, { ...props }));
...
var Controls2 = {
  ...
  text: TextControl,
  file: FilesControl
}, NoControl = () => React17.createElement(React17.Fragment, null, "-"), ArgControl = ({ row, arg, updateArgs, isHovered, storyId }) => {
...
var manager_default = addons.register(ADDON_ID6, (api) => {
  if (globalThis?.FEATURES?.controls) {
    ...
    addons.add(ADDON_ID6, {
      ...
      type: types.PANEL,
      paramKey: PARAM_KEY5,
      render: ({ active }) => !active || !api.getCurrentStoryData() ? null : React28.createElement(AddonPanel, { active, hasHorizontalScrollbar: !0, hasScrollbar: !0 }, React28.createElement(ControlsPanel, { saveStory, createStory }))
...
import React33, { useCallback as useCallback7, useEffect as useEffect10, useMemo as useMemo5, useState as useState13 } from "react";
...
function ActionLogger2({ active, api }) {
  let [actions, setActions] = useState13([]), expandLevel = useParameter2(PARAM_KEY)?.expandLevel ?? 1, clearActions = useCallback7(() => {
    api.emit(CLEAR_ID), setActions([]);
  }, [api]), addAction = useCallback7((action) => {
  ...
  }, []), handleStoryChange = useCallback7(() => {
    actions.length > 0 && actions[0].options.clearOnStoryChange && clearActions();
  }, [actions, clearActions]);
  useEffect10(() => (api.on(EVENT_ID, addAction), api.on(STORY_CHANGED2, handleStoryChange), () => {
...
import React55, { Fragment as Fragment3, memo as memo4, useEffect as useEffect17, useMemo as useMemo6, useRef as useRef6, useState as useState20 } from "react";
import {
  FORCE_REMOUNT,
...
var TRIGGER_TEST_RUN_REQUEST = `${ADDON_ID9}/trigger-test-run-request`, TRIGGER_TEST_RUN_RESPONSE = `${ADDON_ID9}/trigger-test-run-response`;

// src/component-testing/components/InteractionsPanel.tsx
import * as React54 from "react";
import { srOnlyStyles as srOnlyStyles2, styled as styled32 } from "storybook/theming";
...
import React36, { useEffect as useEffect11, useState as useState14 } from "react";
...
import React45, { useEffect as useEffect15, useState as useState18 } from "react";
...
import React44, { useEffect as useEffect14, useState as useState17 } from "react";

// ../../node_modules/@devtools-ds/tree/dist/esm/index.js
import React40, { useState as useState15, useEffect as useEffect12 } from "react";
...
import React41, { useState as useState16, useEffect as useEffect13 } from "react";
...
var Container3 = styled32.div(({ theme: theme3 }) => ({
  ...
  position: "relative"
}), InteractionsHeading = styled32.h3(srOnlyStyles2), InteractionsList = styled32.ol({
  margin: 0,
  padding: 0
...
import React58, { Fragment as Fragment4, memo as memo5, useCallback as useCallback8 } from "react";
...
import React60, { useCallback as useCallback9, useEffect as useEffect18 } from "react";
...
var Tool = () => {
  let [globals, updateGlobals] = useGlobals3(), { measureEnabled } = globals || {}, api = useStorybookApi8(), toggleMeasure = useCallback9(
    () => updateGlobals({
...
import React63, { memo as memo6, useCallback as useCallback10, useEffect as useEffect19 } from "react";
...
var OutlineSelector = memo6(function() {
  let [globals, updateGlobals] = useGlobals4(), api = useStorybookApi9(), isActive = [!0, "true"].includes(globals[PARAM_KEY4]), toggleOutline = useCallback10(
    () => updateGlobals({
      [PARAM_KEY4]: !isActive
    }),
...
import { useCallback as useCallback11, useEffect as useEffect20, useMemo as useMemo7, useRef as useRef7 } from "react";
import { useGlobals as useGlobals5, useParameter as useParameter5, useStorybookApi as useStorybookApi10 } from "storybook/manager-api";
var URL_VALUE_PATTERN = /^([0-9]{1,4})([a-z]{0,4})-([0-9]{1,4})([a-z]{0,4})$/, VIEWPORT_MIN_WIDTH = 40, VIEWPORT_MIN_HEIGHT = 40, cycle = (viewports, current, direction = 1) => {
  ...
  let global3 = normalizeGlobal(globals?.[PARAM_KEY2]), userGlobal = normalizeGlobal(userGlobals?.[PARAM_KEY2]), storyGlobal = normalizeGlobal(storyGlobals?.[PARAM_KEY2]), storyHasViewport = PARAM_KEY2 in storyGlobals, primaryGlobal = storyHasViewport ? storyGlobal : userGlobal, secondaryGlobal = storyHasViewport ? userGlobal : storyGlobal, value = primaryGlobal?.value ?? secondaryGlobal?.value ?? global3?.value, isRotated = primaryGlobal?.isRotated ?? secondaryGlobal?.isRotated ?? global3?.isRotated ?? !1, keys = Object.keys(options), isLocked = disable || PARAM_KEY2 in storyGlobals || !keys.length, [match, vx, ux, vy, uy] = value?.match(URL_VALUE_PATTERN) || [];
  if (match) {
  ...
  let api = useStorybookApi10(), { viewMode } = api.getUrlState(), lastSelectedOption = useRef7(), parameter = useParameter5(PARAM_KEY2), [globals, updateGlobals, storyGlobals, userGlobals] = useGlobals5(), { options = MINIMAL_VIEWPORTS, disable = !1 } = parameter || {}, { name, type, width, height, value, option, isCustom, isDefault, isLocked, isRotated } = parseGlobals(
    globals,
    storyGlobals,
    userGlobals,
    options,
  ...
  return useEffect20(() => {
    parameter && option && (Object.hasOwn(options, option) ? lastSelectedOption.current = option : (lastSelectedOption.current = void 0, PARAM_KEY2 in storyGlobals || update({ value: void 0, isRotated: !1 })));
  }, [parameter, storyGlobals, options, option, update]), useEffect20(() => {
...
var Dimensions = styled33.div(({ theme: theme3 }) => ({
  ...
  let { name, value, isDefault, isLocked, options: viewportMap, reset, select } = useViewport(), options = useMemo8(
