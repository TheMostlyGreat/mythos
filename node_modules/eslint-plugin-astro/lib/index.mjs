...
try {
	...
	if (tsESLintParser = createRequire(path.join(cwd, "__placeholder__.js"))("@typescript-eslint/parser")) hasTypescriptEslintParser = true;
} catch {}
//#endregion
...
const rule = createRule("no-prerender-export-outside-pages", {
	...
	create(context) {
		if (!getSourceCode$1(context).parserServices?.isAstro) return {};
		const filename = getFilename$1(context);
...
function requireUserLocal(id) {
	try {
		const cwd = process.cwd();
		return createRequire(path.join(cwd, "__placeholder__.js"))(id);
	} catch {
		return null;
	}
}
