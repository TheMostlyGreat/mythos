...
class ParameterType {
    ...
    static checkParameterTypeName(typeName) {
        if (!ParameterType.isValidParameterTypeName(typeName)) {
            throw new CucumberExpressionError_js_1.default(`Illegal character in parameter name {${typeName}}. Parameter names may not contain '{', '}', '(', ')', '\\' or '/'`);
        }
    }
...
function regexpSource(regexp) {
    ...
    for (const flag of ['g', 'i', 'm', 'y']) {
        if (flags.indexOf(flag) !== -1) {
            throw new CucumberExpressionError_js_1.default(`ParameterType Regexps can't use flag '${flag}'`);
        }
    }
