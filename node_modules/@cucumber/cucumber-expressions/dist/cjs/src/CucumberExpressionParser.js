...
const CucumberExpressionTokenizer_js_1 = __importDefault(require("./CucumberExpressionTokenizer.js"));
 ...
 * parameter := '{' + text* + '}'
 */
const parseParameter = parseBetween(Ast_js_1.NodeType.parameter, Ast_js_1.TokenType.beginParameter, Ast_js_1.TokenType.endParameter, [parseName]);
/*
 * optional := '(' + option* + ')'
 * option := optional | parameter | text
 */
const optionalSubParsers = [];
const parseOptional = parseBetween(Ast_js_1.NodeType.optional, Ast_js_1.TokenType.beginOptional, Ast_js_1.TokenType.endOptional, optionalSubParsers);
optionalSubParsers.push(parseOptional, parseParameter, parseText);
/*
 * alternation := alternative* + ( '/' + alternative* )+
 */
function parseAlternativeSeparator(_expression, tokens, current) {
...
const parseAlternation = (expression, tokens, current) => {
    ...
    if (!result.ast.some((astNode) => astNode.type === Ast_js_1.NodeType.alternative)) {
        return { consumed: 0, ast: [] };
    }
 ...
 */
const parseCucumberExpression = parseBetween(Ast_js_1.NodeType.expression, Ast_js_1.TokenType.startOfLine, Ast_js_1.TokenType.endOfLine, [parseAlternation, parseOptional, parseParameter, parseText]);
class CucumberExpressionParser {
    parse(expression) {
        const tokenizer = new CucumberExpressionTokenizer_js_1.default();
...
function parseBetween(type, beginToken, endToken, parsers) {
    return (expression, tokens, current) => {
        ...
        const ast = [new Ast_js_1.Node(type, result.ast, undefined, start, end)];
        return { consumed, ast };
    };
}
