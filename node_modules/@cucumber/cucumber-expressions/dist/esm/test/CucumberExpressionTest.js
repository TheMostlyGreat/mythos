...
describe('CucumberExpression', () => {
    for (const path of glob.sync(`${testDataDir}/cucumber-expression/matching/*.yaml`)) {
        ...
        it(`matches ${path}`, () => {
            ...
            if (expectation.expected_args !== undefined) {
                ...
                assert.deepStrictEqual(JSON.parse(JSON.stringify(matches ? matches.map((value) => value.getValue(null)) : null, (_key, value) => {
    ...
    it('unmatched optional groups have undefined values', () => {
        const parameterTypeRegistry = new ParameterTypeRegistry();
        parameterTypeRegistry.defineParameterType(new ParameterType('textAndOrNumber', /([A-Z]+)?(?: )?([0-9]+)?/, null, (s1, s2) => [s1, s2], false, true));
