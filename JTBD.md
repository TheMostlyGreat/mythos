...
- **Place forks where the story turns, weighted to what the Writer cares about.** A fork can only honestly live where the plot genuinely branches — _structure_ picks the candidate sites, never the Writer's preferences (forcing one onto a favorite topic with no real branch is the cosmetic fork above). But among the honest candidates, the engine weights toward the dimensions the Writer revealed at intake (the romance they elaborated, the world they dug into) and frames the options in those terms. Crucially, a dimension they _ignored_ still surfaces now and then: placement leans toward revealed interest without collapsing onto it, so the story can still surprise them with something they didn't know they'd care about.
- **The user can always request a fork** ("let me steer here") on top of the ones the engine surfaces.

> **Open build risk:** tiered cadence requires the engine to _detect_ where a genuine "interesting decision" lives in the plot, rather than hardcoding "fork at chapter 3." Detection is structure-first (where does the plot actually branch?), then weighted by revealed interest per the bullet above. That detection is the non-trivial part; the user-requested fork is the fallback.
...
Two rules keep this from going wrong, both straight from the research on self-adjusting interfaces: adaptation moves **slowly and predictably** (no thrash from one stray click), and every adaptation is **elective** — a fork is always skippable and a fork is always summonable, so the Writer can override the engine's read at any moment without losing agency. "Reader is a mode of the Writer" resolves here: lean-back and lean-forward aren't modes anyone selects — they're emergent positions on a curve the story discovers from how the Writer plays it. And the curve doesn't start blind: per _Intake and forks are one grammar_ below, how the Writer answered the opening interview seeds the cadence chapter one opens at.

### Intake and forks are one grammar
